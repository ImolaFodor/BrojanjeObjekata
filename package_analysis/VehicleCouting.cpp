...
VehiclePosition VehicleCouting::getVehiclePosition(const CvPoint2D64f centroid)
{
	...
	/*if (laneOrientation == LO_HORIZONTAL)
	{
		...
		if (centroid.x > FAV1::roi_x0)
		{
			cv::putText(img_input, "STATE: B", cv::Point(10, 200), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));
...
void VehicleCouting::process()
{
	...
	if (ROI_OK)
	{
		...
		{*/
			...
			cv::putText(img_input, "Ispod linije", cv::Point(FAV1::roi_x0, FAV1::roi_y0 + 22), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));
			cv::putText(img_input, "Iznad linije", cv::Point(FAV1::roi_x0, FAV1::roi_y0 - 12), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255));
		//}
	}
	...
	std::string countABstr = "Broj krastavaca: " + std::to_string(countAB);
	...
	if (showOutput)
		cv::imshow("Brojanje krastavaca", img_input);
